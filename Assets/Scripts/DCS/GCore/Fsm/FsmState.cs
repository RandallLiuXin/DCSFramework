...
namespace Galaxy.Fsm
{
    ...
    public abstract class FsmState<T> where T : class
    {
        ...
        protected internal virtual void OnEnter(IFsm<T> fsm, FsmEnterParams enterParams = null)
        {
        }
        ...
        protected void SubscribeEvent(int eventId, FsmEventHandler<T> eventHandler)
        {
            ...
            {
                throw new GalaxyException("Event handler is invalid.");
            }
        ...
        /// <param name="fsm">有限状态机引用。</param>
        /// <param name="enterParams">初始化参数。</param>
        protected void ChangeState<TState>(IFsm<T> fsm, FsmEnterParams enterParams = null) where TState : FsmState<T>
        {
            Fsm<T> fsmImplement = (Fsm<T>)fsm;
            if (fsmImplement == null)
            {
                throw new GalaxyException("FSM is invalid.");
            }

            fsmImplement.ChangeState<TState>(enterParams);
        }
        ...
        /// <param name="enterParams">初始化参数。</param>
        protected void ChangeState(IFsm<T> fsm, Type stateType, FsmEnterParams enterParams = null)
        {
            Fsm<T> fsmImplement = (Fsm<T>)fsm;
            if (fsmImplement == null)
            {
            ...
            fsmImplement.ChangeState(stateType, enterParams);
        }
