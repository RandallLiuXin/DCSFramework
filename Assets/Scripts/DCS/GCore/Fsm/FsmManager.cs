...
namespace Galaxy.Fsm
{
    ...
    /// 有限状态机管理器。
    /// </summary>
    internal sealed class FsmManager : GalaxyModule, IFsmManager
    {
        ...
        public bool HasFsm(Type ownerType)
        {
            if (ownerType == null)
            {
                throw new GalaxyException("Owner type is invalid.");
            }
        ...
        public IFsm<T> CreateFsm<T>(string name, T owner, params FsmState<T>[] states) where T : class
        {
            if (HasFsm<T>(name))
            {
                throw new GalaxyException(Utility.Text.Format("Already exist FSM '{0}'.", Utility.Text.GetFullName<T>(name)));
            }
