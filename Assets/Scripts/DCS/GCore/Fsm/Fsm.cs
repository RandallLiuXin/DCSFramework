...
namespace Galaxy.Fsm
{
    ...
    internal sealed class Fsm<T> : FsmBase, IFsm<T> where T : class
    {
            ...
            : base(name)
        {
            if (owner == null)
            {
                throw new GalaxyException("FSM owner is invalid.");
            }

            if (states == null || states.Length < 1)
            {
                throw new GalaxyException("FSM states is invalid.");
            }
            ...
            foreach (FsmState<T> state in states)
            {
                ...
                {
                    throw new GalaxyException(Utility.Text.Format("FSM '{0}' state '{1}' is already exist.", Utility.Text.GetFullName<T>(name), stateName));
                }
        ...
        public void Start<TState>(FsmEnterParams enterParams = null) where TState : FsmState<T>
        {
            if (IsRunning)
            {
                throw new GalaxyException("FSM is running, can not start again.");
            }

            FsmState<T> state = GetState<TState>();
            if (state == null)
            {
                throw new GalaxyException(Utility.Text.Format("FSM '{0}' can not start state '{1}' which is not exist.", Utility.Text.GetFullName<T>(Name), typeof(TState).FullName));
            }

            m_CurrentStateTime = 0f;
            m_CurrentState = state;
            m_CurrentState.OnEnter(this, enterParams);
        }
        ...
        /// <param name="enterParams">初始化参数。</param>
        public void Start(Type stateType, FsmEnterParams enterParams = null)
        {
            if (IsRunning)
            {
                throw new GalaxyException("FSM is running, can not start again.");
            ...
            if (stateType == null)
            {
                throw new GalaxyException("State type is invalid.");
            }

            if (!typeof(FsmState<T>).IsAssignableFrom(stateType))
            {
                throw new GalaxyException(Utility.Text.Format("State type '{0}' is invalid.", stateType.FullName));
            }

            FsmState<T> state = GetState(stateType);
            if (state == null)
            {
                throw new GalaxyException(Utility.Text.Format("FSM '{0}' can not start state '{1}' which is not exist.", Utility.Text.GetFullName<T>(Name), stateType.FullName));
            }
        ...
        public void GetAllStates(List<FsmState<T>> results)
        {
            if (results == null)
            {
                throw new GalaxyException("Results is invalid.");
            }
        ...
        public void FireEvent(object sender, int eventId)
        {
            ...
            {
                throw new GalaxyException("Current state is invalid.");
            }
        ...
        public bool HasData(string name)
        {
            ...
            {
                throw new GalaxyException("Data name is invalid.");
            }
        ...
        /// <param name="enterParams">初始化参数。</param>
        public void ChangeState<TState>(FsmEnterParams enterParams = null) where TState : FsmState<T>
        {
            ChangeState(typeof(TState), enterParams);
        }
        ...
        /// <param name="enterParams">初始化参数。</param>
        public void ChangeState(Type stateType, FsmEnterParams enterParams = null)
        {
            if (m_CurrentState == null)
            {
                throw new GalaxyException("Current state is invalid.");
            ...
            if (state == null)
            {
                throw new GalaxyException(Utility.Text.Format("FSM '{0}' can not change state to '{1}' which is not exist.", Utility.Text.GetFullName<T>(Name), stateType.FullName));
            }
        ...
        /// 获取有限状态机的所有数据。
        /// </summary>
        /// <returns>有限状态机的所有数据</returns>
        public Dictionary<string, Variable> GetAllData()
        {
            return m_Datas;
        }
    }
