...
namespace Galaxy.Mold
{
    public sealed partial class DataTypeProcessor
    {
        ...
        private DataTypeCodeGenerator m_CodeGenerator;

        public DataTypeProcessor(string moldFileName, Encoding encoding, int nameRow, int typeRow, int? defaultValueRow, int? commentRow, int contentStartRow, int idColumn)
        {
            if (string.IsNullOrEmpty(moldFileName))
            {
                throw new GalaxyException("Data table file name is invalid.");
            }

            if (!moldFileName.EndsWith(".txt"))
            {
                throw new GalaxyException(Utility.Text.Format("Data table file '{0}' is not a txt.", moldFileName));
            }

            if (!File.Exists(moldFileName))
            {
                throw new GalaxyException(Utility.Text.Format("Data table file '{0}' is not exist.", moldFileName));
            }

            string[] lines = File.ReadAllLines(moldFileName, encoding);
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                {
                    throw new GalaxyException(Utility.Text.Format("Raw Column is '{1}', but line '{0}' column is '{2}'.", i.ToString(), rawColumnCount.ToString(), rawValue.Length.ToString()));
                }
            ...
            if (nameRow < 0)
            {
                throw new GalaxyException(Utility.Text.Format("Name row '{0}' is invalid.", nameRow.ToString()));
            }

            if (typeRow < 0)
            {
                throw new GalaxyException(Utility.Text.Format("Type row '{0}' is invalid.", typeRow.ToString()));
            }

            if (contentStartRow < 0)
            {
                throw new GalaxyException(Utility.Text.Format("Content start row '{0}' is invalid.", contentStartRow.ToString()));
            }

            if (idColumn < 0)
            {
                throw new GalaxyException(Utility.Text.Format("Id column '{0}' is invalid.", idColumn.ToString()));
            }

            if (nameRow >= rawRowCount)
            {
                throw new GalaxyException(Utility.Text.Format("Name row '{0}' >= raw row count '{1}' is not allow.", nameRow.ToString(), rawRowCount.ToString()));
            }

            if (typeRow >= rawRowCount)
            {
                throw new GalaxyException(Utility.Text.Format("Type row '{0}' >= raw row count '{1}' is not allow.", typeRow.ToString(), rawRowCount.ToString()));
            }

            if (defaultValueRow.HasValue && defaultValueRow.Value >= rawRowCount)
            {
                throw new GalaxyException(Utility.Text.Format("Default value row '{0}' >= raw row count '{1}' is not allow.", defaultValueRow.Value.ToString(), rawRowCount.ToString()));
            }

            if (commentRow.HasValue && commentRow.Value >= rawRowCount)
            {
                throw new GalaxyException(Utility.Text.Format("Comment row '{0}' >= raw row count '{1}' is not allow.", commentRow.Value.ToString(), rawRowCount.ToString()));
            }

            if (contentStartRow > rawRowCount)
            {
                throw new GalaxyException(Utility.Text.Format("Content start row '{0}' > raw row count '{1}' is not allow.", contentStartRow.ToString(), rawRowCount.ToString()));
            }

            if (idColumn >= rawColumnCount)
            {
                throw new GalaxyException(Utility.Text.Format("Id column '{0}' >= raw column count '{1}' is not allow.", idColumn.ToString(), rawColumnCount.ToString()));
            }
        ...
        public bool IsIdColumn(int rawColumn)
        {
            ...
            {
                throw new GalaxyException(Utility.Text.Format("Raw column '{0}' is out of range.", rawColumn.ToString()));
            }
        ...
        public bool IsCommentRow(int rawRow)
        {
            ...
            {
                throw new GalaxyException(Utility.Text.Format("Raw row '{0}' is out of range.", rawRow.ToString()));
            }
        ...
        public bool GenerateDataFile(string outputFileName, Encoding encoding)
        {
            ...
            {
                throw new GalaxyException("Output file name is invalid.");
            }
        ...
        public void SetCodeGenerator(DataTypeCodeGenerator codeGenerator)
        {
        ...
        public bool GenerateCodeFile(string outputFileName, Encoding encoding, object userData = null)
        {
            ...
            {
                throw new GalaxyException("You must set code template first.");
            }
