...
namespace Galaxy
{
    ...
    internal sealed partial class EventPool<T> where T : BaseEventArgs
    {
        ...
        public void Subscribe(int id, EventHandler<T> handler)
        {
            ...
            {
                throw new GalaxyException(Utility.Text.Format("Event '{0}' not allow multi handler.", id.ToString()));
            }
            else if ((m_EventPoolMode & EventPoolMode.AllowDuplicateHandler) == 0 && Check(id, handler))
            {
                throw new GalaxyException(Utility.Text.Format("Event '{0}' not allow duplicate handler.", id.ToString()));
            }
            else
            {
                handlers.AddLast(handler);
        ...
        public void Unsubscribe(int id, EventHandler<T> handler)
        {
            ...
            if (!m_EventHandlers.TryGetValue(id, out handlers))
            {
                throw new GalaxyException(Utility.Text.Format("Event '{0}' not exists any handler.", id.ToString()));
            }

            if (!handlers.Remove(handler))
            {
                throw new GalaxyException(Utility.Text.Format("Event '{0}' not exists specified handler.", id.ToString()));
            }
        }
        ...
        private void HandleEvent(object sender, T e)
        {
            ...
            {
                throw new GalaxyException(Utility.Text.Format("Event '{0}' not allow no handler.", eventId.ToString()));
            }
        }
    }
}
