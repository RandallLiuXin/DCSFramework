...
namespace Galaxy.ObjectPool
{
    /// <summary>
    /// 对象池管理器。
    /// </summary>
    internal sealed partial class ObjectPoolManager : GalaxyModule, IObjectPoolManager
    {
        ...
        internal override void Update(float elapseSeconds)
        {
            ...
            {
                objectPool.Value.Update(elapseSeconds);
            }
        }
        ...
        public bool HasObjectPool(Type objectType)
        {
            if (objectType == null)
            {
                throw new GalaxyException("Object type is invalid.");
            }

            if (!typeof(ObjectBase).IsAssignableFrom(objectType))
            {
                throw new GalaxyException(Utility.Text.Format("Object type '{0}' is invalid.", objectType.FullName));
            }
        ...
        public bool HasObjectPool(string fullName)
        {
            ...
            {
                throw new GalaxyException("Full name is invalid.");
            }
        ...
        public bool HasObjectPool(Predicate<ObjectPoolBase> condition)
        {
            if (condition == null)
            {
                throw new GalaxyException("Condition is invalid.");
            }
        ...
        public bool DestroyObjectPool<T>(IObjectPool<T> objectPool) where T : ObjectBase
        {
            if (objectPool == null)
            {
                throw new GalaxyException("Object pool is invalid.");
            }
        ...
        private IObjectPool<T> InternalCreateObjectPool<T>(string name, bool allowMultiSpawn, float autoReleaseInterval, int capacity, float expireTime, int priority) where T : ObjectBase
        {
            ...
            {
                throw new GalaxyException(Utility.Text.Format("Already exist object pool '{0}'.", Utility.Text.GetFullName<T>(name)));
            }
        ...
        private ObjectPoolBase InternalCreateObjectPool(Type objectType, string name, bool allowMultiSpawn, float autoReleaseInterval, int capacity, float expireTime, int priority)
        {
            ...
            {
                throw new GalaxyException(Utility.Text.Format("Already exist object pool '{0}'.", Utility.Text.GetFullName(objectType, name)));
            }
