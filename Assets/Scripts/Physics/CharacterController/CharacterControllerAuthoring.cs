...
public struct CharacterControllerInput : IComponentData
{
    ...
    //SIN03 only
    public float3 RootMotionDeltaPos; 
}
    ...
    struct CharacterControllerJob : IJobEntityBatch
    {
            ...
            ref CharacterControllerInternalData ccInternalData, ref float3 linearVelocity)
        {
            ...
            // Turning
            {
                ...
                float vertical = ccInternalData.Input.Movement.y;
                bool haveInput = (math.abs(horizontal) > float.Epsilon) || (math.abs(vertical) > float.Epsilon);
                if (haveInput && math.abs(DeltaTime) > float.Epsilon)
                {
                    float currentAngle = ccInternalData.CurrentRotationAngle;
                    float targetAngle = ccInternalData.Input.Looking;

                    float temp = Mathf.Repeat(targetAngle - currentAngle, 2 * Mathf.PI);
                    if (temp > Mathf.PI)
                    {
                        temp -= 2 * Mathf.PI;
                    }
                    var offestAngle = math.sign(temp) * math.min(math.abs(temp), ccComponentData.RotationSpeed * DeltaTime);

                    ccInternalData.Velocity.Angular = -offestAngle / DeltaTime * up;
                    ccInternalData.CurrentRotationAngle += offestAngle;
                }
            ...
            // Apply input velocities
            {
                ...
                //SIN03 only
                linearVelocity += math.mul(quaternion.AxisAngle(up, ccInternalData.CurrentRotationAngle), ccInternalData.Input.RootMotionDeltaPos / DeltaTime);
                ccInternalData.Input.RootMotionDeltaPos.x = 0;
                ccInternalData.Input.RootMotionDeltaPos.y = 0;
                ccInternalData.Input.RootMotionDeltaPos.z = 0;
            }
        }
